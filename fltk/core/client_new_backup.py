        ...
        if self.mal and self.config.attack_client is not None:
            ...
            for epoch in range(num_epochs):
                ...
                for i, (inputs, labels) in enumerate(self.dataset.get_train_loader(), 0):
                    ...
                    # Determine if defense should be applied this epoch
                    ...
                    # ============================================================
                    # LeadFL Defense Implementation (Following Paper Algorithm 1)
                    # ============================================================
                    if self.config.defense == "LeadFL" and defense_half:
                        # --- Step 1: Compute gradient at current parameters θ_i ---
                        self.optimizer.zero_grad()
                        outputs = self.net(inputs)
                        loss = self.loss_function(outputs, labels)
                        running_loss += loss.item()
                        ...
                        # Store g_i (gradient at θ_i)
                        grads_at_theta = {}
                        for name, param in self.net.named_parameters():
                            if param.requires_grad and param.grad is not None:
                                grads_at_theta[name] = param.grad.clone()
                        
                        # --- Step 2: Temporary SGD step: θ̃ = θ - η * g_i ---
                        with torch.no_grad():
                            for param in self.net.parameters():
                                if param.requires_grad and param.grad is not None:
                                    param.data -= self.config.lr * param.grad
                        
                        # --- Step 3: Compute gradient at new position θ̃ ---
                        self.optimizer.zero_grad()
                        outputs_new = self.net(inputs)
                        loss_new = self.loss_function(outputs_new, labels)
                        loss_new.backward()
                        
                        # --- Step 4 & 5: Estimate Hessian and compute R ---
                        # H̃ ≈ (g̃ - g) / η
                        # R = clip(I - η * H̃, q)
                        for name, param in self.net.named_parameters():
                            if param.requires_grad and param.grad is not None and name in grads_at_theta:
                                # Hessian diagonal approximation
                                grad_diff = param.grad - grads_at_theta[name]
                                hessian_diag = grad_diff / self.config.lr
                                
                                # R = I - η * H̃ (element-wise)
                                R = torch.ones_like(param) - self.config.lr * hessian_diag
                                
                                # Clip R to [-q, q] where q = pert_strength
                                R_clipped = torch.clamp(R, -self.config.pert_strength, self.config.pert_strength)
                                
                                # --- Step 6: Final update: θ = θ̃ - η * α * R ---
                                # α = regular_weight
                                with torch.no_grad():
                                    param.data -= self.config.lr * self.config.regular_weight * self.regular_schedule * R_clipped
                                
                                # Track regularization loss for logging
                                regular_loss += torch.norm(R_clipped).item()
                        
                        # Store for Hessian tracking metrics
                        for name, param in self.net.named_parameters():
                            if param.requires_grad and param.grad is not None:
                                old_gradient_mine[name] = param.grad.clone()
                                old_params[name] = param.clone()

                    # ============================================================
                    # Original implementations for other defenses
                    # ============================================================
                    elif self.config.defense in ["myWBC", "LeadFLnorm"]:
                        # zero the parameter gradients
                        self.optimizer.zero_grad()
                        outputs = self.net(inputs)
                        loss = self.loss_function(outputs, labels)
                        ...
                        # No defense or other defenses (WBC, LDP, none)
                        self.optimizer.zero_grad()
                        outputs = self.net(inputs)
                        loss = self.loss_function(outputs, labels)
                        running_loss += loss.item()
                    ...
                    # Track gradient changes for Hessian metrics (for all defenses)
                    if i != 0:
                        changed_ele_num = 0
                        all_ele_num = 0
                        changed_magnitude = 0
                        for name, p in self.net.named_parameters():
                            if 'weight' in name and name in old_gradient:
                    ...
                    if i % self.config.log_interval == 0:
                        self.logger.info(
                            f'[{self.id}] [{epoch:d}, {i:5d}] loss: {running_loss / self.config.log_interval:.3f}')
                        final_running_loss = running_loss / self.config.log_interval
                        final_regular_loss = regular_loss / self.config.log_interval if regular_loss > 0 else 0.0
                        running_loss = 0.0
                        regular_loss = 0.0
                        # break
